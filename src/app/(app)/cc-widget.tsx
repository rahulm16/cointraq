  ...
  return (
    <Card>
      <div className="flex items-center gap-2 text-[14px] font-semibold text-text-primary">
        <CreditCard size={16} strokeWidth={1.5} className="text-text-secondary" />
        {account.name}
      </div>
      ...
      <div className="flex items-baseline gap-2 mt-3">
        <span className="tnum text-[28px] text-text-primary">{formatINR(statement.unbilled)}</span>
      ...
      <div className="flex items-center gap-2 mt-3.5 text-[13px] text-text-secondary flex-wrap">
        Last bill <span className="tnum font-medium text-text-primary">{formatINR(statement.lastStatement)}</span>
        {badge}
        {statement.paidStatus === "partial" && (
